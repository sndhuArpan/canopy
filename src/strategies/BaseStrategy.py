...
class BaseStrategy:
    def __init__(self, exchange, name):
        ...
        self.startTimestamp = MarketTiming(self.exchange).get_market_start_time() # When to start the strategy. Default is Market start time
    ...
    def run(self):
        ...
        if MarketTiming(self.exchange).is_market_closed_for_the_day():
            logging.warning("%s: Not going to run strategy as market is closed.", self.getName())
            return

        now = datetime.now()
        if now < MarketTiming(self.exchange).get_market_start_time():
            MarketTiming(self.exchange).wait_till_market_opens(self.getName())
